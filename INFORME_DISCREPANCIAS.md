...
| 🔵 INFO / NO VERIFICABLE | 10 | 10 documentados (#33–#42) · 0 pendientes |
...
| 41 | 19.11 | `KitchenOrderUpdated` no se dispara en `addItem`/`updateItem`/`saveOrderNotes` (sí en removeItem/sendToKitchen/charge/split/etc.) | Aclarado §19.11: el evento se dispara en la mayoría de métodos; addItem/updateItem/saveOrderNotes no (el poll del KDS cada 5s los cubre). **Documentado** | `app/Http/Controllers/Restaurant/RestaurantController.php` | ✅ RESUELTO (doc) |
| 42 | 20.16 | "lite/ws mismo commit entre 5.2.0 y 5.3.0" y "template despatch2022 +5 líneas" no verificables sin historial de vendor | Documentado en §20.16: la actualización fue necesaria para mantener la facturación electrónica (SUNAT); esos detalles requieren historial git de vendor (composer no lo incluye); sí se verifica v5.3.0 en composer.lock. **Documentado** | `vendor/greenter/*`, `composer.lock` | ✅ DOCUMENTADO (NO VERIFICABLE) |
...
| 2026-08-02 | #41 | `DOCUMENTACION_SISTEMA.md` §19.11 | Opción A (solo doc): aclarado que `KitchenOrderUpdated` se dispara en la mayoría de métodos que modifican pedidos (lista) y que `addItem`/`updateItem`/`saveOrderNotes` NO lo disparan (el polling del KDS cada 5s los cubre). Solo documentación. |
| 2026-08-02 | #42 | `DOCUMENTACION_SISTEMA.md` §20.16 | Añadida nota: la actualización a Greenter v5.3.0 fue necesaria para mantener la facturación electrónica (SUNAT); los detalles "mismo commit" (lite/ws) y "5 líneas nuevas" (template) no son verificables desde el repo (requieren historial git de vendor; composer no lo incluye); sí se verifica v5.3.0 en composer.lock. Ítem NO VERIFICABLE documentado. |

---

## ✅ AUDITORÍA COMPLETA — INFORME_DISCREPANCIAS.md

**Resultado final (42 ítems):**
- 🔴 **ALTA (1):** 1/1 resuelto (#1)
- 🟠 **MEDIA (20):** 20/20 resueltos (#2–#21)
- 🟡 **BAJA (12):** 11/11 resueltos (#22–#32; el resumen inicial decía 12, pero la tabla lista 11)
- 🔵 **INFO/NO VERIFICABLE (10):** 10/10 documentados/verificados (#33–#42)

**Todos los ítems accionables están resueltos.** Quedan únicamente notas documentales e ítems NO VERIFICABLES (requieren historial git de vendor o configuración externa del entorno del cliente).
