...
| 2.3 | Agosto 2026 | Módulo Ingresos y Gastos: tabla `cash_movements` + `total_ingresos`/`total_egresos` en `cashregisters`; permiso `manage_cash_movements` (admin+cajero); requiere caja abierta. **Saldo Final de Efectivo** = `monto_apertura(ingreso) + ventas_efectivo + total_ingresos − total_egresos − monto_cierre(egreso)` (apertura=ingreso, cierre=egreso; Yape/Plin/Tarjeta virtuales, solo informativo; positivo=sobra, negativo=caja negativa marcada en rojo; sin "diferencia") en web, PDF A4, ticket 80mm y ESC/POS. |
