...
| 2.3 | Agosto 2026 | Módulo Ingresos y Gastos: tabla `cash_movements` + `total_ingresos`/`total_egresos` en `cashregisters`; permiso `manage_cash_movements` (admin+cajero); requiere caja abierta. **Saldo Final de Efectivo** = `ventas_efectivo + total_ingresos − total_egresos` (sin apertura; Yape/Plin/Tarjeta virtuales, solo informativo; marcar en rojo si negativo = faltante; sin "diferencia") en web, PDF A4, ticket 80mm y ESC/POS. |
