...
| 2.3 | Agosto 2026 | Módulo Ingresos y Gastos: tabla `cash_movements` + `total_ingresos`/`total_egresos` en `cashregisters`; permiso `manage_cash_movements` (admin+cajero); requiere caja abierta; conciliación en cierre (`saldo_esperado`/`diferencia` **solo con efectivo**; Yape/Plin/Tarjeta son virtuales y no cuentan como dinero en caja) en web, PDF A4, ticket 80mm y ESC/POS. |
