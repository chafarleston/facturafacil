...
- `eliminar_ventas_productos.bat` — menú para eliminar ventas y/o productos (scripts `eliminar-ventas-productos/clean_ventas.php` y `clean_productos.php` vía tinker con `base_path`). `clean_ventas` además **pone el stock de todos los productos (normales y compuestos) en 0**. Crea backup MySQL previo en `eliminar-ventas-productos/backup/` y pide confirmación `SI`. DESTRUCTIVO.
...
- **Facturación (fixes de seguridad — no romper)**: `buildInvoice`/NC/ND usan `invoice_items.precio_venta` (total línea CON IGV, consistente en POS/Restaurante/Manual) para montos; NO re-dividir `precio_unitario` (varía base/con IGV según origen). `InvoiceController::sendToSunat` bloquea re-envío si `sunat_estado` ya es ACEPTADO/ENVIADO/ANULADO (guard solo en `PosController` históricamente). NC de boleta va por `'07'` (no `'08'`). `checkTicketStatus` solo marca ACEPTADO a `sunat_estado=ENVIADO` (no revierte anulados) y en rechazo pasa los documentos del ticket a PENDIENTE (recuperables). Reintento de notas 07/08 vía `SummaryService::retryNote()` (requiere `invoices.docu_referencia`, migración 2026_09_21_000002). **Scheduler en Laravel 13**: las tareas van en `routes/console.php` con `Schedule::command()` (`print:process-queue` cada min, `sunat:download-padron` dom 02:00, `sunat:send-daily-summary` 09:00/12:00, `sunat:check-summaries` 09:05/09:20/12:05/12:20, `sunat:verify-unresolved` 12:06) — `App\Console\Kernel::schedule()` NO se ejecuta en Laravel 11+. **Envío de facturas (01) es INMEDIATO y sincrónico** en `PosController::store` / `RestaurantController::createInvoiceFromItems` / `InvoiceController::store`, con reserva de estado (`ENVIADO` transitorio) y `default_socket_timeout` (~12s); las NO enviadas quedan `PENDIENTE` para reintento manual. **Boletas (03) NO se autodisparan**: se agrupan por `fecha_emision` en el Resumen Diario de la mañana siguiente (09:00, reintento 12:00); una enviada manualmente queda `ENVIADO` → excluida. **Alerta SUNAT** (`sunat_alerts` + campana/modal en navbar para admin/cajero): si a las 12:06 quedan boletas `PENDIENTE` de días previos (`sunat:verify-unresolved`), y se auto-resuelve vía `SunatAlertService::evaluate()` al no quedar pendientes. Pendiente: reintento de notas con ticket rechazado, correlativo secuencial de bajas, emisor por `invoice->company_id`.
- **Permisos (roles)**: `cajero` tiene `view_invoices`, `create_invoices`, `send_sunat` (envía a SUNAT), `view_cashregisters` + `open_cashregister` (abre caja, NO la cierra), `manage_cash_movements` (registra ingresos/gastos de caja), `view_restaurant`/`manage_orders`, `authorize_cancel_orders` (autoriza eliminaciones/anulaciones de cocina con su propia contraseña), `manage_order_merge` (fusiona mesas) y **gestión completa de productos y categorías** (`view/create/edit/delete_products` y `view/create/edit/delete_categories`). `view_cashregister_history` (historial de cajas) lo tiene solo `admin` por defecto. El rol `user` NO tiene `view_invoices` ni `view_cashregisters` (no ve Comprobantes ni Caja); solo lectura de productos (`view_products`/`view_categories`). `superadmin` es un valor reservado en la lógica (`isSuperAdmin()`, `hasPermission`, `IsAdmin`), NO un rol de la BD; el acceso total efectivo es vía `admin`. Todos los permisos de productos/categorías se pueden añadir/quitar por rol desde Roles/Permisos.
...
- **Fusionar Mesas**: botón "Fusionar" en el pedido (permiso `manage_order_merge`, admin/cajero por defecto). Une el pedido de la mesa origen con el de una **mesa ocupada** destino: mueve todos los items (tal cual, conservando estado de cocina), recalcula totales (suma de ambos), libera la mesa origen (AVAILABLE) y conserva el `order_number` del destino. **Bloquea si** el origen o el destino tienen items facturados (`paid_invoice_id`) o si es pedido kiosko. Mover (`moveTable`) conserva su lógica simple (no fusiona).
...
- **Bloqueo del sistema**: opción "Bloquear/Desbloquear Sistema" en Empresa, **visible solo para el dueño** (`config('app.lock_owner_email')`, default `rcharles84@gmail.com`). Flag persistente en tabla `settings` (`system_locked`, con caché). Middleware `SystemLock` (append al grupo `web`) responde **503** con la vista `errors/sistema_bloqueado` para **todo** excepto rutas `login`, `logout`, `password/*` y `system-lock.toggle`. Ruta `POST /system-lock/toggle` (`SystemLockController`) valida el email del dueño → 403 si no. La pantalla de bloqueo muestra botón "Desbloquear" solo al dueño.
...
- **mPDF downloads**: nunca usar `$pdf->Output($nombre, 'D')` — mPDF hace `echo` (Buffer.php) y rompe headers de Laravel ("Cannot modify header information ... Buffer.php:57"). Usar `'S'` + `response($pdf->Output($nombre,'S'),200)->header('Content-Type','application/pdf')->header('Content-Disposition','attachment; filename="'.$nombre.'"')`. Afecta a `CashRegisterController::pdf/ticketPdf`, `AttendanceReportController::exportPdf`, `ProductController::inventoryReportPdf`. Los de Greenter usan `'S'` (seguros).
